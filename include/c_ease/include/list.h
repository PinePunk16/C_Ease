// list.h
    ...
    void List_append_var(List* this, const var value) {
        (this -> size)++;

        if(this -> value == NULL) this -> value = malloc((this -> size) * sizeof(var));
        else this -> value = realloc(this -> value, (this -> size) * sizeof(var));

        if(this -> value == NULL) exit(1); //TBU

        (this -> value)[(this -> size) - 1] = value;
    }
    #define List_append(this,value)         List_append_var(&this, auto(value))

    string List_to_string(List this) {
        size_t buffer_size = 2;
        string buffer = malloc(buffer_size * sizeof(char));
